...
class WsServer:
    ...
    def __init__(
        ...
        ai_service_url: typing.Optional[str] = None,
        path: typing.Optional[str] = None,
    ):
        self.loop = asyncio.get_event_loop()
        # Base config; callers may pass per-request overrides in messages
        app_cfg = load_app_config(host=host, port=port, ai_service_url=ai_service_url, path=path)
        whisper_cfg = AsyncWhisperTranscriber.WhisperRuntimeCfg(
            ...
            max_single_word_repeats=app_cfg.whisper.max_single_word_repeats,
            drop_repeated_only_segments=app_cfg.whisper.drop_repeated_only_segments,
        )
        ...
        # Bind using the final values from resolved AppConfig
        self.host = app_cfg.net.host
        self.port = int(app_cfg.net.port)
...
def main():
    ...
    parser.add_argument("--host", dest="host", default=None, help="Host/IP to bind (overrides config)")
    parser.add_argument(
        "--port",
        dest="port",
        type=int,
    ...
    parser.add_argument(
        ...
        dest="ai_service_url",
        default=None,
        help="WebSocket URL to bind, e.g. ws://0.0.0.0:9772 (overrides host/port and config)",
    )
    args = parser.parse_args()

    async def _main():
        server = WsServer(host=args.host, port=args.port, ai_service_url=args.ai_service_url)
        await server.start()
